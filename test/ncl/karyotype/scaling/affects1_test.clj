...
(ns ncl.karyotype.scaling.affects1_test
  (:use [clojure.test])
  (:require
...
(deftest Get-Band
  (let [band (#'ncl.karyotype.scaling.affects1/get-band 1 "p36.3")]
...
(deftest Not-Breakpoint?
  ...
  (let [inputs [h/HumanChromosome1Bandp36.31 h/HumanChromosome1Bandp10
    ...
    (doseq [input inputs]
      (is (instance?
           ...
           input))
      (is (h/band? input))
      (is (false? (#'ncl.karyotype.scaling.affects1/not-breakpoint? input input))))
    (is (every? true? (map #(#'ncl.karyotype.scaling.affects1/not-breakpoint? %1 %2)
                           inputs
                           (replace
  ...
  (is (thrown?
       AssertionError
       "HumanChromosome"
       (#'ncl.karyotype.scaling.affects1/not-breakpoint?
        h/HumanChromosomeBand
        h/HumanChromosome))))
